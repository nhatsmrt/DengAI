...
class SimpleNet:
    def __init__(self, n_features, n_classes = 135, keep_prob = 0.8, use_gpu = False):
        self._n_classes = n_classes
        self._keep_prob = keep_prob
        self._use_gpu = use_gpu
        if use_gpu:
            with tf.device('/device:GPU:0'):
                self.create_network(n_features)
        else:
    ...
    def create_network(self, n_features):
        ...
        self._fc1 = self.feed_forward(self._X_norm, name = "fc1", inp_channel= n_features, op_channel = 40)
        self._fc2 = self.feed_forward(self._fc1, name = "fc2", inp_channel= 40, op_channel = self._n_classes)
...
# Adapt from Stanford's CS231n Assignment 3
        ...
        for e in range(epochs):
            ...
            for i in range(int(math.ceil(Xd.shape[0] / batch_size))):
                ...
                if i < int(math.ceil(Xd.shape[0] / batch_size)) - 1:
                    ...
                    val_loss, val_acc = session.run([self._mean_loss, accuracy], feed_dict = feed_dict)
    ...
    # Train:
    def fit(self, X, y, num_epoch = 64, batch_size = 8, weight_save_path = None, weight_load_path = None, plot_losses = False):
